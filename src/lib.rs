...
/// Lint an SSH config file by path.
...
";
    ...
    fn has_errors_true_when_error_present() {
        let findings = vec![Finding::error("test", "bad", model::Span::new(1))];
        assert!(has_errors(&findings));
    }

    #[test]
    fn has_errors_false_when_only_warnings() {
        let findings = vec![Finding::warning("test", "meh", model::Span::new(1))];
        assert!(!has_errors(&findings));
    }
